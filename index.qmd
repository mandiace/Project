...
Why did we choose these predictors? According to a study conducted by [Kulachi et al. (2023)](https://www.semanticscholar.org/paper/Factors-Influencing-Voting-Decision%3A-A-Literature-Kulachai-Lerdtomornsakul/7c0c5c51420091efe7fc2dcd8cc413a93c4590ce), voting behavior is very dynamic. People's voting patterns depend on a culmination of numerous factors. Some factors shown to impact voting behavior that are included in this analysis are economic, gender, ethnicity and race, and age variables. Thus, we pulled predictors within these groupings. Other factors that have been shown to influence voting behaviors, like health care experiences, media influences, are difficult to estimate at an individual level. Thus, these are much more difficult to estimate at a county level. Due to the complexities in estimating these variables and limited data access, we do not include these variables in our analysis.
...
#countymap_winner + map_propwhite + map_propblack + map_prophispanic
  
```

Tried to create maps to visualize a relationship between white and non-white ethnic makeups and vote differentials but they're not very explanatory considering the vast swath of counties that voted red. Open to any ideas on how to improve the maps or to gut them altogether. Also saved your computer by not rendering the maps as it took mine about five minutes to do so.
...
train |>
  ...
  geom_smooth(mapping = aes(x = prop_less_than_hs,
              ...
              color = "grey",
              linetype = "dashed") + 
  labs(title = "Association between proportion of county with less than high school educ. and margin",
...
train |>
  ...
  geom_smooth(mapping = aes(x = prop_hs_grad,
              ...
              color = "grey",
              linetype = "dashed") + 
  labs(title = "Association between proportion of county with high school educ. and margin",
...
train |>
  ...
  geom_smooth(mapping = aes(x = prop_some_college_associates,
              ...
              color = "grey",
              linetype = "dashed") + 
  labs(title = "Association between proportion of county with some college or associate's and margin",
...
train |>
  ...
  geom_smooth(mapping = aes(x = prop_bachelors_higher,
              ...
              color = "grey",
              linetype = "dashed") + 
  labs(title = "Association between proportion of county with a bachelor's and margin",
...
NEED TO JUSTIFY SPECIFIC VARIABLES, MEDIAN HOUSING COST
...
We chose to imputate using knn to ensure that any missing predictors had an estimation. We chose knn instead of mean imputation because we felt this would provide an estimate that was based on more evidence (or observations), hopefully improving the accuracy of our missing data.
...
We logged two predictors, median income and median housing costs. This is because median income and housing costs have a wide range of values. By logging the variables, we improve the symmetry of our data and make it more linearly associated with the outcome variable. It helps reduce skewness of our data and helps us meet the key assumption of linearity in statistical modeling.
...
We normalized all predictors because of inherent differences across our types of predictors. We use variables that have a range of scales. For example, median age has a scale ranging from 0 to 100. A variable like housing costs, might vary from 100,000 dollars to 1 million. Having predictors with various scales could make some predictors influence our model estimation more than other predictors. By normalizing the predictors, we attempt to ensure that all predictors are considered equally in the model estimation.
...
The LASSO model out-of-sample RMSE is \[6,162, 6,553\]
...
The Regression Tree model out-of-sample RMSE is \[36,661, 41,626\]
...
The KNN model out-of-sample RMSE is \[14,286, 15,241\]

The LASSO model has the lowest out-of-sample RMSE.
...
electoral_votes_state <- predictions |> 
  group_by(state_name) |> 
...
finaldata2024 |> 
  ...
  select(diff_current, diff_previous) |> 
  view()

```

The model seems to be fairly accurate in predicting the vote differentials per county but, more importantly, in predicting the outcome of the 2024 presidential election. Although the RMSE of the final model may be of concern, the aggregate state outcomes nonetheless convey an accurate prediction of the 2024 presidential election, correctly anticipating a Donald Trump victory. Our model suggests that, despite the assumingly unprecedented likelihood of Trump's re-election backed by popular media, his victory could have been predicted utilizing county demographic, economic, and political factors that we included in our LASSO model.

## Limitations

### Inability to logarithmize diff_current
