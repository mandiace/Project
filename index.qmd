...
elections2024 <- elections2024 |> 
  filter(!county_fips %in% c(11001, 11002, 11003, 11004, 11005, 11006, 11007, 11008)) |> # dc data inputted manually from census burea information
  bind_rows(dc_data)
...
#countymap_diff + map_propwhite + map_propblack + map_prophispanic
...
train |>
  mutate(winner = ifelse(diff_current < 0, "dem", "rep")) |> 
  group_by(winner) |>
  summarize("Max margin" = abs(max(diff_current)),
            "Min margin" = abs(min(diff_current)),
            "Mean margin" = abs(mean(diff_current)),
...
train |>
  ...
  group_by(winner) |>
  summarize("Max margin" = abs(max(diff_previous)),
            "Min margin" = abs(min(diff_previous)),
            "Mean margin" = abs(mean(diff_previous)),
...
The LASSO model out-of-sample RMSE is [6,369, 6,553]
...
Benefit and importance of model
...
Redundant code below vvvvvvvvvvv
